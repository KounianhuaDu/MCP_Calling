# from mcp zero
# MCP-Zero/MCP-zero/matcher.py at master · xfey/MCP-Zero
import json
import numpy as np
import re
import time
...
class ToolMatcher:
    ...
    def setup_openai_client(self, base_url: str, api_key: str) -> None:
        ...
        self.openai_client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")
    ...
    def get_embedding(self, text: str, max_retries: int = 3) -> Optional[List[float]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                outputs = self.openai_client.embeddings.create(model='/ext0/jxliu/models/Qwen3-Embedding-0.6B', input=[text])
