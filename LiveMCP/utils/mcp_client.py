...
class MCPClient:
    ...
    async def cleanup_server(self, server_id: str):
        """Stop and cleanup resources for a single server.

        This function avoids awaiting the current task (which would deadlock
        if called from the task itself). It will set the stop event, then
        cancel the background task and wait with a timeout. Finally it removes
        entries from internal dicts safely.
        """
        # Guard missing entries
        stop_evt = self.stop_event.get(server_id)
        task = self.task.get(server_id)

        if stop_evt is None and task is None and server_id not in self.sessions:
            # Nothing to do
            return

        # Signal the session to stop if possible
        if stop_evt is not None and not stop_evt.is_set():
            try:
                stop_evt.set()
            except Exception:
                logger.debug("Failed to set stop_event for %s", server_id, exc_info=True)

        # If we have a task and it's not the current task, cancel and await with timeout
        ...
        if task is not None and task is not current:
            if not task.done():
                try:
                    ...
                    logger.debug("Failed to cancel task for %s", server_id, exc_info=True)
            try:
                await asyncio.wait_for(task, timeout=5)
            except asyncio.TimeoutError:
                logger.warning("Timed out waiting for task %s to finish", server_id)
            except Exception:
                logger.debug("Error while awaiting task %s", server_id, exc_info=True)

        # Remove session from cache if present
        try:
            self.sessions.pop(server_id, None)
        except Exception:
            logger.debug("Failed to pop session for %s", server_id, exc_info=True)

        # Remove bookkeeping entries
