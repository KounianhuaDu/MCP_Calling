...
class ReAct(BaseAgent):
    ...
    async def _execute(
        ...
        """
        logger.info(f"[ReAct] Enter execute, question={message}")
        ...
        for iter_num in range(self._config.max_iterations):
            ...
            try:
                ...
                print("=="*60)
                # —— 在 try: 之前或开头 —— #
                raw_response = response  # 原始内容（勿改动）
                # 清洗前，仅做温和的外层 codefence/前后空白清理
                response = raw_response.strip().strip('`').strip()
                if response.startswith("json"):
                    response = response[4:].strip()

                # 尝试从包含杂质的文本中“提取”第一个 JSON 大括号片段，作为兜底
                ...
                candidate = response
                if not (response.startswith("{") and response.rstrip().endswith("}")):
                    candidate = _extract_json_blob(response)
