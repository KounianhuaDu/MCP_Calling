...
""".strip()'''
OUTPUT_FORMAT_PROMPT = """You are an intelligent reasoning agent.

You MUST respond in EXACTLY two channels in this order and nothing else:

<|channel|>commentary
- Think step by step but DO NOT reveal chain-of-thought.
- If you need to call a tool, output EXACTLY ONE JSON object on its own line (no extra text):
{{"server":"{{server_name}}","tool":"{{tool_name}}","arguments":{{arguments_json}}}}
- The JSON must be minified on a single line. No markdown, no code fences, no explanations.

<|channel|>final
- Provide the concise final answer for the user. No JSON here. No markdown/code fences.

STRICT RULES:
- No text before <|channel|>commentary.
- No text after the <|channel|>final block.
- The two channel tags must each appear ONCE and ONLY ONCE, in the above order.
- If you output anything outside the two channels, your answer is invalid.
"""
...
""".strip()'''
TOOL_RESPONSE_SUMMARIZER_PROMPT = """You will summarize the tool response for the user. Follow the channel format.

<|channel|>commentary
- No tool calls here.
- Briefly note what was extracted (do not reveal chain-of-thought).

<|channel|>final
- Extract all information from the tool response that is relevant to the user's context.
- Try to maintain the original structure when useful.
- Use fewer than 500 words.
- No markdown and no JSON; write plain sentences.
